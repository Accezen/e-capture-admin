...
<body>
            ...
            <div class="col-sm-12 container-main pre-load">
                <div class="heading">
                    <div class="heading-label">
                        <span>Archived Drivers</span>
                    </div>

                </div>

                <span class="reg-count">No. of archived passengers:</span>
                <span class="reg-count num" id = "d_count2"></span>

                <div class="table-container mt-5 p-1" id="result2">
    ...
    <script type="module">
        ...
        const colRef = collection(db, "passenger");
        ...
        $(document).ready(function() {
            ...
            let deactPassRef = collection(db, "passenger");
            const q = query (deactPassRef, where("status", "==", "inactive"));
            ...
            onSnapshot(q,(snapshot) => {
                ...
                $("#result").html("");
                $("#result").append(` <table id="passenger" class="table table-responsive table-hover ">
                        <thead class="ulo">
                            <tr>
                                <th>Name</th>
                                <th>Address</th>
                ...
                snapshot.forEach((passenger) => {
                    ...
                    var status = passenger.data().status;
                    ...
                    $("#passengerData").append(`
                            <tr>
                                <td class="clickable" data-href="passenger_details.php?id=${passenger.id}">${passenger.data().first_name}&nbsp${passenger.data().last_name}</td>
                                <td class="clickable" data-href="passenger_details.php?id=${passenger.id}">${passenger.data().address}</td>
                                <td class="clickable" data-href="passenger_details.php?id=${passenger.id}">${passenger.data().mobile}</td>
                                <td class="clickable" data-href="passenger_details.php?id=${passenger.id}">${passenger.data().email }</td>
                                <td>
                                    <button class="btn btn-secondary activate-btn" data-passenger-id="${ passenger.id }"><i class="fa fa-lg fa-archive m-1"></i>Unarchive</button>
                                </td>
                            </tr>  
                ...
                async function activatePassenger(passengerId){
                    const docRef = doc(db, 'passenger', passengerId);
                    const data = {
                        status: "active"
                    };
                    ...
                    Swal.fire({
                            ...
                            text: 'Are you sure do you want to Activate this Account?',
            ...
            let deactDriRef = collection(db, "driver");
            const qq = query (deactDriRef, where("status", "==", "inactive"));

            onSnapshot(qq,(snapshot) => {

                let user2 = []
                snapshot.docs.forEach((doc)=> {
                user2.push({...doc.data(), id: doc.id})
                });

                var count = user2.length;

                $("#result2").html("");
                $("#result2").append(` <table id="driver" class="table table-responsive table-hover ">
                        <thead class="ulo">
                            <tr>
                                <th>Name</th>
                                <th>Vehicle Type</th>
                ...
                snapshot.forEach((driver) => {
                    ...
                    var rem = ( ratings <= 3 ) ? 'warning' 
                    : (ratings = "No Ratings Yet") ? 'neutral'
                    : 'fine';
                    ...
                    $("#tblDriver").append(`
                            <tr>
                                ...
                                <td>
                                <button class="btn btn-secondary activate-btn-driver" data-driver-id="${ driver.id }"><i class="fa fa-lg fa-archive m-1"></i>Unarchive</button>
                                </td>
                            </tr>  
                ...
                $(document).on('click', '.activate-btn-driver', function(){
                ...
                async function activateDriver(driverId){
                    const dRef = doc(db, 'driver', driverId);
                    const data = {
                        status: "active"
                    };
